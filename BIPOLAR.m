function [G_BIPOLAR, b_val, phaseFinal]=BIPOLAR(grad,T_ECHO,bvalue_target,dt,G_Max,S_Max,Gvec,T_90,T_RF)
    ...
    %
    %
    %This function computes the BIPOLAR gradient waveform and its bValue, and
    %the final phase.
    %
    %
    ...
    %
    % OUTPUTS : G_BIPOLAR - Final BIPOLAR gradient waveform
    %            b_val - Final b-value   
    %            phaseFinal - remaining phase of the G_BIPOLAR waveform
    ...
    if nargin==0 || nargin==1 || nargin==2 || nargin==3 || nargin==4
      ...
      if nargin<3
          bvalue_target = 000;    % s/mm2 if bvalue_target==0 it does not derate the waveform
      end
       if nargin<4
    ...
    if tECHO/2+T_90*1e-3/dt>n
        fprintf('BIPOLAR: The sequence times are not valid. T_EPI is longer than T_Diff\n');
        G_BIPOLAR=grad;
        b_val=-1;
        phaseFinal=-1;
        return;
    ...
    grad(t2+nSlope+1:t2+ceil(n_after_RF/2)-nSlope-1)=G_Max;
    grad(t2+ceil(n_after_RF/2)-nSlope:t2+ceil(n_after_RF/2)-1)=slopeDOWN;
    grad(t2+ceil(n_after_RF/2)+1:t2+ceil(n_after_RF/2)+nSlope)=-slopeUP;
    grad(t2+ceil(n_after_RF/2)+nSlope+1:end-nSlope-1)=-G_Max;
    grad(end-nSlope:end-1)=-slopeDOWN;
    
    grad(t2-n_after_RF-ceil(RFTime/(dt*1e3)):t2-ceil(RFTime/(dt*1e3)))=grad(t2:end);
    ...
    DESCRIPTION = ['bValue: ' num2str(round(b_val)) ', TE: ' num2str(TE) '---Gmax: ' num2str(max(G_BIPOLAR)/Gvec)  '---SRmax: ' num2str(max(abs(D*G_BIPOLAR/Gvec)))  ];
...
%     plot(G_BIPOLAR,'LineWidth',2); hold on; %plot(start);
...
%     subplot(3,1,3);
%     plot(abs(D*G_BIPOLAR/Gvec),'LineWidth',2);ylabel('dG/dt'); %slew rate
